  ...
  while (1)
  {
    ...
    if (key == 0x31)
    {
      ...
      */
      ...
      //在第一次烧程序的时候不需要留着数据备份，在之后需要保留A程序之前的版本了，代表需要落后A一个版本，每次烧录A的时候把程序传给B在开始烧
      //这里还有一个问题，烧第一次不需要备份，烧第二次呢，因为在第一次的时候直接给了B，在第二次，相当于已经有了，不需要烧
      if(Upload_flag)Upload_flag++;
      else if(Upload_flag > 2)
      {
        ...
        //写进去
        FLASH_If_Write(&flashdestination_N, (uint32_t *)&New_CRC, sizeof(New_CRC) / 4)  == 0;
        FLASH_If_Write(&flashdestination_L, (uint32_t *)&Last_CRC, sizeof(Last_CRC) / 4)  == 0;
        //这里应该上锁了
        HAL_FLASH_Lock();
        ...
        while(1)
        {
          ...
          flash_transmit(A_addr,B_addr,4U,1,&New_CRC);
          //是不是应该算一下CRC
      ...
      //这里是对A烧录程序
      SerialDownload();
      //这里是烧录A，多久烧给B?刚开始肯定不行，B需要留着上次的备份

      //就再这里，第一次执行的时候，直接给B，后面需要烧录
      /*还应该想到的问题
      1、如果去备份没有成功，此时应该还在Upload_BUSY
      2、如果A的数据出现了问题，B应该传给A
      */
      //烧录B,因为是第一次烧程序，这个时候A是第一个版本，直接传给B，不需要备份
      if(Upload_flag == 0)
      {
        Last_CRC.State = Upload_BUSY;
        Temp1 = New_CRC; 
