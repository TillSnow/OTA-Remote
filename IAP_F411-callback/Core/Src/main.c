...
int main(void)
{
  ...
  New_CRC = *(AB_CRC *)CRC_addr;
  Last_CRC = *(AB_CRC *)(CRC_addr + 4);
  ...
  if(RTC->BKP0R == 3)
  {
    ...
    while(1)
    {
      flash_transmit(B_addr,A_addr,3U,1,&Last_CRC);
  ...
  //如果掉电了，应该是不能直接接着烧录的，因为不知道到那一步了，
  if(New_CRC.State == Upload_BUSY)
  {
    uint8_t key1;
    //应该有一个选项，选择重新烧录或者退回老版本
    key1 = GetKey();
