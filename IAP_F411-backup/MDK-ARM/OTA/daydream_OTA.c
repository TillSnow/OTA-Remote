...
void JUMP_TO_Addr()
{
    ...
    Jump_To_App = (pFunction)Reset_Addr;
...
{   
    ...
    give_Data = New_CRC.len / 512; 
    //但是如果有1025个字节，就会导致漏
    if(Remain = New_CRC.len % 512 > 0)give_Data += 1;
    while(give_Data--)
    {
        //将地址里面的数据存储进数组
        ...
        flash_Rx.VoltageRange = FLASH_VOLTAGE_RANGE_4;
        //存进数组之后应该擦除要传输的
        if(HAL_FLASHEx_Erase(&flash_Rx,&SectorError) != HAL_OK)return 1;
        //擦除了扇区之后应该是往里面写了
